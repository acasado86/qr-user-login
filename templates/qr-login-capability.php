...
<form method="post" action="<?php echo admin_url('users.php?page=qr-login-capability') ?>">
    ...
    <p><label for="<?php echo $role ?>"><input type="checkbox" id="<?php echo $role ?>" name="qr_login_roles[]" value="<?php echo $role ?>" <?php checked($role_object->has_cap('qr_login')) ?>/><?php echo $role ?></label></p>
    <?php endforeach; ?>
